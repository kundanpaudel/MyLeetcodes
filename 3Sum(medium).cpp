...
public:
    vector<vector<int>> threeSum(vector<int>& nums) {
        ...
        if(size<3){ // We need to have at least 3 elements to calculate the sum.
        ...
        for (int i = 0; i<size-2; i++){ 
            if(nums[i]>0){ // If our ith term is greater than zero that means there are no negative numbers after it. So we break the loop.
                break;
            }
            if(i>0 && nums[i-1]==nums[i]){ // If we find same value elements, we just continue until we find a different one.
                continue;
            }
            ...
            while (j<k){ // We loop until j and k cross paths
                ...
                if(sum > 0){ // If sum is greater than zero we move the second pointer backwards.
                    k--;
                }
                else if(sum < 0){ // If sum is less than zero, we move first pointer forwards.
                    j++;
                }
