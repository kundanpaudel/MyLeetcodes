...
public:
    bool isAnagram(string s, string t)
    {
        ...
        { // If length of s and t are different they cannot be anagrams.
            return false;
        }
        else
        {
            for (int i = 0; i < s.length(); i++)
            {                 // Iterate through length of both strings s = t.
            ...
            for (auto x : s)
            { // For each character in string s (you can also use t).
                if (strS[x] != strT[x])
                {                 // Check if each character have same frequency in each map.
