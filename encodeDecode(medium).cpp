...
public:
    ...
    string encode(vector<string> &strs) {
        ...
        for(int i = 0; i<strs.size(); i++){ // Loop through the strs vector
    ...
    vector<string> decode(string &str) {
        ...
        while(i<str.length()){ // Loop until end of the string is reached.
            int j = i; // Store value of i in j to calculate the string length later.
            while(str[j]!='#'){ // Keep looping until '#' is encountered.
