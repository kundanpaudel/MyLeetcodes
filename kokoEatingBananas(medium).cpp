...
public:
    int minEatingSpeed(vector<int> &piles, int h)
    {
        ...
        for(auto p:piles){ // We iterate through the given list
        ...
        while(lowSpeed<=highSpeed){ 
            ...
            for(auto p:piles){ // We looop through the list again
                // And calculate the total hours taken to eat at midSpeed
                totalHours += ceil((double) p/midSpeed);
            }
            if(totalHours <= h){ // If the total hours taken is less than or equal to the target hours
            ...
            }else{ // If the hours taken is larger than provided target hour
