...
public:
    string minWindow(string s, string t) {
        ...
        while(j<s.length()){ // Moving a pointer until the end of the string creating windows
            if (tMap[s[j]]>0){ // If the current character in s is in tMap, then we decrese the counter
                counter --;
            }
            ...
            while(counter == 0){ // When we have found all characters of t in substring window of s
                if(j-i < minLength){ // If the length of current substring is less than the minimum length we have so far
