...
*/
    bool isPalindrome(string s) {
        ...
        while(i<j){ // While the pointers have not crossed paths.
            while(!isalnum(s[i]) && i<j){ // While the current character for pointer i is not alphanumeric and i<j
                i++; // Increase the pointer to next character in the string
            }
            while(!isalnum(s[j]) && i<j){ // While current character for pointer j is not alphanumeric and i<j
                j--; // Decrease the pointer to previous character in the string
            }
            if(tolower(s[i])!=tolower(s[j])){ // Once we have found alphanumeric characters in both starting and end pointer we compare them.
